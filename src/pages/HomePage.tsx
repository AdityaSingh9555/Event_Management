...
import { formatDateShort, formatCurrency, getCategoryTheme } from '@/lib/utils'
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-12 px-4 max-w-6xl mx-auto">
        <div className={`relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 text-white border border-white/10 transform transition-all duration-500 ${isCopied ? 'scale-[1.03] shadow-[0_40px_80px_-12px_rgba(16,185,129,0.6)] ring-2 ring-emerald-500' : 'shadow-[0_20px_50px_-12px_rgba(168,85,247,0.5)] hover:scale-[1.01] hover:shadow-[0_30px_60px_-12px_rgba(168,85,247,0.6)]'}`}>
          {/* Decorative elements */}
          ...
          <div className="relative p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-10">
            ...
            <div className="hidden lg:flex w-72 h-72 relative items-center justify-center shrink-0">
              ...
              <div className={`relative w-full h-full backdrop-blur-2xl rounded-full border shadow-2xl flex items-center justify-center p-8 transition-all duration-500 ${isCopied ? 'bg-emerald-500/20 border-emerald-400/50 scale-110' : 'bg-white/10 border-white/20'}`}>
                 <div className={`absolute transition-all duration-500 ease-in-out transform ${isCopied ? 'scale-0 opacity-0 rotate-180' : 'scale-100 opacity-100 rotate-0'}`}>
                   <Gift className="w-32 h-32 text-pink-300 drop-shadow-2xl" />
                 </div>
                 <div className={`absolute transition-all duration-500 ease-in-out transform ${isCopied ? 'scale-100 opacity-100 rotate-0 animate-bounce' : 'scale-0 opacity-0 -rotate-180'}`}>
                   <PartyPopper className="w-32 h-32 text-emerald-300 drop-shadow-2xl" />
                 </div>
              </div>
            </div>
          </div>
